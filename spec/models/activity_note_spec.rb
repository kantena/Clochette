require 'spec_helper'

describe ActivityNote do
  before(:each) do

    @valid_attributes = {
      :customer_id => 1,
      :user_id => 1,
      :working_days => 10,
      :month => 6,
      :year => 2010
    }

    c1, c2 = Factory(:customer, :name => 'Vinci'), Factory(:customer, :name => 'Cour des comptes')
    dev1, dev2, dev3 = Factory(:kantenien, :name =>'nicolas'), Factory(:kantenien, :name => 'philippe'), Factory(:kantenien, :name => 'damien')

    Factory(:activity_note, :customer => c1, :user => dev2, :working_days => 10 )
    Factory(:activity_note, :customer => c1, :user => dev3, :working_days => 5 )
    Factory(:activity_note, :customer => c2, :user => dev1, :working_days => 25 )

  end

  it "should create a new instance given valid attributes" do
    ActivityNote.create!(@valid_attributes)
  end

  it "should have a client" do
    activity_note = Factory(:activity_note)
    assert_kind_of Customer, activity_note.customer
  end

  it "should have a User" do
    activity_note = Factory(:activity_note)
    assert_kind_of User, activity_note.user
  end

  it "should expect a month among 1 and 12" do
    assert_raise(ActiveRecord::RecordInvalid) do
      ActivityNote.create!(:month => 0)
    end
    assert_raise(ActiveRecord::RecordInvalid) do
      ActivityNote.create!(:month => 13)
    end
  end

  it "should expect a year less than current year" do
    assert_raise(ActiveRecord::RecordInvalid) do
      ActivityNote.create!(:year => -2000)
    end
    assert_raise(ActiveRecord::RecordInvalid) do
      ActivityNote.create!(:year => 2040)
    end
  end

  describe "Contrustion de notes à partir d'une liste d'evenement pour un equipier" do
    it "avant la contruction de nouvelle notes, on supprime les anciennes notes du mois" do

      societe = Factory(:customer, :name => 'Coya & Co')
      societe_2 = Factory(:customer, :name => 'Autre compagnie')
      societe_3 = Factory(:customer, :name => 'Une dernière compagnie')
      dev = Factory(:kantenien, :name =>'nfe')

      Factory(:activity_note, :customer => societe, :user => dev, :working_days => 10, :month => 7, :year => 2010 )
      Factory(:activity_note, :customer => societe_2, :user => dev, :working_days => 5, :month => 7, :year => 2010 )
      Factory(:activity_note, :customer => societe_3, :user => dev, :working_days => 25, :month => 7, :year => 2010 )

      ActivityNote.import dev.id
      notes = ActivityNote.find_all_by_user_id dev.id
      assert_equal 0, notes.size
    end

    it "on contruis de nouvelles notes a partir de la liste fournie par le parser" do
      societe = Factory(:customer, :name => 'Coya & Co')
      societe_2 = Factory(:customer, :name => 'CdC')
      societe_3 = Factory(:customer, :name => 'Une dernière compagnie')
      dev = Factory(:kantenien, :name =>'nfe')

      Factory(:activity_note, :customer => societe, :user => dev, :working_days => 10, :month => 7, :year => 2010 )
      Factory(:activity_note, :customer => societe_2, :user => dev, :working_days => 5, :month => 7, :year => 2010 )
      Factory(:activity_note, :customer => societe_3, :user => dev, :working_days => 25, :month => 7, :year => 2010 )

      list = []
      event = Event.new
      event.user = "nfe"
      event.title = "CdC"
      event.start_time = Date.civil(2010,7,1)
      event.end_time = Date.civil(2010,7,27)
      list << event
      AgendaGoogle.any_instance.stubs(:list_event).returns(list)
      ActivityNote.import dev.id
      
      ActivityNote.import dev.id
      notes = ActivityNote.find_all_by_user_id dev.id
      assert_equal 1, notes.size
    end

    it "on contruis de nouvelles notes a partir de la liste fournie par le parser avec un nom de société inconnu" do
      societe = Factory(:customer, :name => 'Coya & Co')
      societe_2 = Factory(:customer, :name => 'CdC')
      societe_3 = Factory(:customer, :name => 'Une dernière compagnie')
      dev = Factory(:kantenien, :name =>'nfe')

      Factory(:activity_note, :customer => societe, :user => dev, :working_days => 10, :month => 7, :year => 2010 )
      Factory(:activity_note, :customer => societe_2, :user => dev, :working_days => 5, :month => 7, :year => 2010 )
      Factory(:activity_note, :customer => societe_3, :user => dev, :working_days => 25, :month => 7, :year => 2010 )

      list = []
      event = Event.new
      event.user = "nfe"
      event.title = "X"
      event.start_time = Date.civil(2010,7,1)
      event.end_time = Date.civil(2010,7,27)
      list << event
      AgendaGoogle.any_instance.stubs(:list_event).returns(list)
      ActivityNote.import dev.id

      ActivityNote.import dev.id
      notes = ActivityNote.find_all_by_user_id dev.id
      assert_equal 0, notes.size
    end
  end
end
